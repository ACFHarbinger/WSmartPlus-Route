...
class StateTSP(NamedTuple):
    ...
    @staticmethod
    def initialize(loc, edges, cost_weights=None, dist_matrix=None, visited_dtype=torch.uint8):
    ...
    def get_final_cost(self):
        ...
        return self.w_length * self.lengths + self.w_length * (self.loc[self.ids, self.first_a, :] - self.cur_coord).norm(p=2, dim=-1)
    ...
    def update(self, selected):
        ...
        return self._replace(first_a=first_a, prev_a=prev_a, visited_=visited_, lengths=lengths, cur_coord=cur_coord, i=self.i + 1)
