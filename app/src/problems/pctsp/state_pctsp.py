...
class StatePCTSP(NamedTuple):
    ...
    @staticmethod
    def initialize(input, edges, cost_weights=None, dist_matrix=None, visited_dtype=torch.uint8, stochastic=False):
        depot = input['depot']
        loc = input['loc']
        ...
        return StatePCTSP(
            ...
            w_prize=1 if cost_weights is None else cost_weights['prize'],
            w_penalty=1 if cost_weights is None else cost_weights['penalty'],
            edges=edges,
    ...
    def get_final_cost(self):
        ...
        return self.w_length * self.lengths + self.w_penalty * self.cur_total_penalty
