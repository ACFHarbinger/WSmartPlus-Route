...
from .bgpbt_kernels import CasmoKernel


# Adapted from https://github.com/xingchenwan/bgpbt/blob/main/hpo/utils.py
...
def get_dim_info(cs: CS.ConfigurationSpace, x, return_indices=False):
    ...
    for i, variable in enumerate(range(len(x))):
        ...
        if x[variable] != x[variable]:
            continue
        if type(cs[cs.get_hyperparameter_by_idx(variable)]) == CSHP.CategoricalHyperparameter:
            cat_dims.append(cs.get_hyperparameter_by_idx(variable))
            cat_dims_idx.append(i)
        elif type(cs[cs.get_hyperparameter_by_idx(variable)]) in [CSHP.UniformIntegerHyperparameter,
                                                                  CSHP.NormalIntegerHyperparameter]:
            int_dims.append(cs.get_hyperparameter_by_idx(variable))
            int_dims_idx.append(i)
        elif type(cs[cs.get_hyperparameter_by_idx(variable)]) in [CSHP.UniformFloatHyperparameter,
                                                                  CSHP.NormalFloatHyperparameter]:
...
def construct_bounding_box(cs: CS.ConfigurationSpace, x, tr_length, weights=None, ):
    ...
    for i, dim in enumerate(x):
        if np.isnan(x[i]) or i >= len(cs):
            ...
            if type(hp) == CSHP.CategoricalHyperparameter:
                ...
                if type(hp) in [CSHP.UniformIntegerHyperparameter, CSHP.NormalIntegerHyperparameter,
                                CSHP.NormalFloatHyperparameter, CSHP.UniformFloatHyperparameter]:
...
def get_start_point(cs: CS.ConfigurationSpace, x_center, frozen_dims: List[int] = None, return_config=False):
    ...
    for i in range(new_config_array.shape[0]):
        ...
        if type(cs[param_name]) == CSHP.CategoricalHyperparameter:
            new_config_array[i] = np.random.choice(
                range(len(cs[param_name].choices)))
        elif type(cs[param_name]) in [CSHP.UniformIntegerHyperparameter, CSHP.NormalIntegerHyperparameter] and cs[param_name].lower == 0 and cs[param_name].upper == 1:   # binary
