...
from src.utils.definitions import VEHICLE_CAPACITY
...
class StateSDVRP(NamedTuple):
    ...
    VEHICLE_CAPACITY = VEHICLE_CAPACITY  # Hardcoded
    ...
    @staticmethod
    def initialize(input, edges, cost_weights=None, dist_matrix=None):
        depot = input['depot']
        loc = input['loc']
        demand = input['demand']
    ...
    def get_final_cost(self):
        ...
        return self.w_length * self.lengths + self.w_length * (self.coords[self.ids, 0, :] - self.cur_coord).norm(p=2, dim=-1)
