...
class Container():
    """
    Overall obejct to deal with container data. Is supports manipulation operations over the container, 
    managing the 
    """
    ...
    def get_keys(self):
        ...
        return {"FILL": list(self.df.keys()), "RECS": list(self.recs.keys()), "INFO": list(self.info.keys())}
    ...
    def get_scan_linear_spline(self, key, interval) -> tuple[pd.DatetimeIndex, np.ndarray]:
        ...
        date_range = pd.date_range(start=self.recs.index[0].round('D'),
                                    end = self.df.index[-1].round('D'), freq=interval)
        
        numeric_data_range = np.array(list(
            map(lambda y: 
            (y-date_range[0]).total_seconds()/(3600*24), date_range)
            ))
    ...
    def get_monotonic_mean_rate_error_splines(self, interval):
        ...
        interval:str,
            ...
            https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-offset-aliases
        """
        assert all(map(lambda x: x in list(self.df.keys()), ['Max', 'Min'])), \
        ...
        upper_spline = np.interp(numeric_data_range, self.df['Num_T'], self.df['Diff_Max'].to_numpy().cumsum())
        lower_spline = np.interp(numeric_data_range, self.df['Num_T'], self.df['Diff_Min'].to_numpy().cumsum())

        self.df.drop(['Min_SHF', 'Max_SHF', 'Num_T', 'Diff_Min', 'Diff_Max'], axis=1, inplace=True)
    ...
    def get_monotonic_mean_rate(self, freq) -> pd.DataFrame:
        ...
        df = pd.Series(data = np.diff(spline),
                                index = data_range[:-1],
                                name = 'Rate')
    ...
    def get_tag(self, window:int, mv_thresh:int, min_days:int, use: str) -> TAG:
        ...
        if (len(self.recs)//len(self.df) >= 1 or
            (self.recs.index[-1] - self.recs.index[0]).days < min_days):
    ...
    def calc_spearman(self, start_idx:int = 0, end_idx:int = -1):
        ...
        with warnings.catch_warnings():
            ...
            groupDF = self.df[self.df.index[self.recs["End_Pointer"].iloc[start_idx]]:
                                self.df.index[self.recs["End_Pointer"].iloc[end_idx]-1]].groupby('Cidx')
    ...
    def calc_avg_dist_metric(self, start_idx:int = 0, end_idx:int = -1):
        ...
        res = groupDF.apply(lambda group: 100 - (group['Max']-group['Min']).sum()/len(group) if len(group) > 0 else 100).to_numpy()
        try:
            if len(self.recs) > 1:
            ...
            print(self.df[self.df.index[self.recs["End_Pointer"].iloc[start_idx]]:
                          self.df.index[self.recs["End_Pointer"].iloc[end_idx]-1]])
    ...
    def calc_max_min_mean(self, start_idx:int = 0, end_idx:int = -1):
        ...
        def min_loop(vec:np.ndarray, mask:np.ndarray) -> tuple[np.ndarray, bool]:
            shf = np.roll(vec, -1); shf[-1] = np.iinfo(shf.dtype).max
        ...
        #min loop
        vec = my_df['Fill'].to_numpy().copy()
        mask =  np.roll((my_df['Rec'] == 0).to_numpy(), -1); mask[-1] = False
        ...
        self.df.loc[self.recs.index[start_idx]:self.recs.index[end_idx], 'Mean'] = \
            (self.df.loc[self.recs.index[start_idx]:self.recs.index[end_idx], 'Min'] +
                self.df.loc[self.recs.index[start_idx]:self.recs.index[end_idx], 'Max'])/2
    ...
    def adjust_one_collection(self, idx, c_trash:int, max_fill:int) -> bool:
        ...
        if collected_trash >= c_trash and self.df['Fill'].iat[new_index+base_index] <= max_fill:
            if self.df['Rec'].iat[new_index + base_index] == 0:
                ...
                self.df.loc[
                    self.df.index[self.recs['End_Pointer'].iat[idx]]:self.df.index[self.recs['End_Pointer'].iat[idx+1]-1],
                    'Cidx'
                ] = self.df['Cidx'].iat[self.recs['End_Pointer'].iat[idx]]

                self.df.loc[
                    self.df.index[self.recs['End_Pointer'].iat[idx+1]]:self.df.index[self.recs['End_Pointer'].iat[idx+2]-1],
                    'Cidx'
            ...
            self.df.at[self.df.index[self.recs['End_Pointer'].iat[idx+1]],'Rec'] = 0
            self.df.loc[
                    self.df.index[self.recs['End_Pointer'].iat[idx]]:self.df.index[self.recs['End_Pointer'].iat[idx+2]-1],
                    'Cidx'
                ] = self.df['Cidx'].iat[self.recs['End_Pointer'].iat[idx]]
