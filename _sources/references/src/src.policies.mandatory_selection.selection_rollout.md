...
````{py:method} _eval_bin(idx: int, collect_today: bool, context: logic.src.interfaces.context.SelectionContext, base_policy: logic.src.interfaces.mandatory_selection.IMandatorySelectionStrategy, horizon: int, n_scenarios: int, bin_cap: float, revenue_kg: float, round_trip_cost: numpy.ndarray, max_fill: float, target_sigma: float) -> float
