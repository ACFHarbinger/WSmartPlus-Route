# {py:mod}`src.policies.other.operators`

```{py:module} src.policies.other.operators
```

```{autodoc2-docstring} src.policies.other.operators
:allowtitles:
```

## Subpackages

```{toctree}
:titlesonly:
:maxdepth: 3

src.policies.other.operators.intra_route
src.policies.other.operators.inter_route
src.policies.other.operators.heuristics
src.policies.other.operators.destroy
src.policies.other.operators.crossover
src.policies.other.operators.repair
src.policies.other.operators.unstringing_stringing
src.policies.other.operators.perturbation
src.policies.other.operators.intensification
```

## Submodules

```{toctree}
:titlesonly:
:maxdepth: 1

src.policies.other.operators.neighborhood
```

## Package Contents

### Data

````{list-table}
:class: autosummary longtable
:align: left

* - {py:obj}`__all__ <src.policies.other.operators.__all__>`
  - ```{autodoc2-docstring} src.policies.other.operators.__all__
    :summary:
    ```
````

### API

````{py:data} __all__
:canonical: src.policies.other.operators.__all__
:value: >
   ['INTENSIFICATION_NAMES', 'INTENSIFICATION_OPERATORS', 'two_opt_steepest', 'two_opt_steepest_profit'...

```{autodoc2-docstring} src.policies.other.operators.__all__
```

````
