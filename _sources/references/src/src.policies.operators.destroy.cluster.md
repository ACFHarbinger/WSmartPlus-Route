# {py:mod}`src.policies.operators.destroy.cluster`

```{py:module} src.policies.operators.destroy.cluster
```

```{autodoc2-docstring} src.policies.operators.destroy.cluster
:allowtitles:
```
...
* - {py:obj}`cluster_removal <src.policies.operators.destroy.cluster.cluster_removal>`
