...
def basic_checkpoint(mocker, tmp_path):
    ...
    mocker.patch(
        'src.pipeline.simulator.checkpoints.ROOT_DIR', 
        new=str(mock_root)
    )
    ...
    from src.pipeline.simulator.checkpoints import SimulationCheckpoint
...
def mock_dependencies(mocker):
    ...
    mocker.patch('src.data.generate_data.generate_tsp_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_vrp_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_pctsp_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_op_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_vrpp_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_wcrp_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_wcvrp_data', return_value=[(None, None)])
    mocker.patch('src.data.generate_data.generate_pdp_data', return_value=[(None, None)])
    
    # 2. Mock the WSR simulator data generator
    mocker.patch('src.data.generate_data.generate_wsr_data', return_value=[(None, None)])
    
    # 3. Mock the save utility
    mocker.patch('src.data.generate_data.save_dataset', return_value=None)
...
def mock_sim_dependencies(mocker, tmp_path, mock_bins_instance):
    ...
    """
    # 1. Patch ROOT_DIR
    mocker.patch('src.pipeline.simulator.simulation.ROOT_DIR', str(tmp_path))
    ...
    mocker.patch('src.pipeline.simulator.simulation.load_depot', return_value=mock_depot)
    mocker.patch('src.pipeline.simulator.simulation.load_simulator_data', 
    ...
    mocker.patch('src.pipeline.simulator.simulation.process_data', 
                 return_value=(mock_proc_data.copy(), mock_proc_coords.copy()))
    mocker.patch('src.pipeline.simulator.simulation.process_model_data', 
    ...
    mocker.patch('src.pipeline.simulator.simulation._setup_dist_path_tup', 
                 return_value=(mock_dist_tup, mock_adj_matrix))
    mocker.patch('src.pipeline.simulator.simulation.compute_distance_matrix', 
                 return_value=np.array([[0,1],[1,0]]))
    mocker.patch('src.pipeline.simulator.simulation.apply_edges', 
                 return_value=('mock_dist_edges', 'mock_paths', 'mock_adj'))
    mocker.patch('src.pipeline.simulator.simulation.get_paths_between_states', 
                 return_value='mock_all_paths')

    # 5. Mock Bins class
    mocker.patch('src.pipeline.simulator.simulation.Bins', return_value=mock_bins_instance)

    # 6. Mock setup functions
    mocker.patch(
        'src.pipeline.simulator.simulation.setup_model',
        return_value=(MagicMock(), MagicMock()) 
    )
    mocker.patch('src.utils.setup_utils.setup_env', 
    ...
    mock_run_day = mocker.patch( # CAPTURE the mock object here
        'src.pipeline.simulator.simulation.run_day', 
    ...
    mocker.patch('src.pipeline.simulator.simulation.SimulationCheckpoint', 
    ...
    mocker.patch('src.pipeline.simulator.simulation.checkpoint_manager', 
                return_value=mock_cm)

    # 9. Mock utilities
    mocker.patch('src.pipeline.simulator.simulation.log_to_json')
    mocker.patch('src.pipeline.simulator.simulation.output_stats',
                 return_value=({}, {})) # mock_log, mock_log_std
    mocker.patch('src.pipeline.simulator.simulation.save_matrix_to_excel')
    mocker.patch('src.pipeline.simulator.simulation.time.process_time', return_value=1.0)
    ...
    mocker.patch(
        'src.pipeline.simulator.simulation.tqdm', 
...
def mock_load_dependencies(mocker):
    ...
    # We must mock it where it's *used*, e.g., 'src.utils.definitions'
    mock_udef = MagicMock()
    ...
    mocker.patch('src.utils.definitions', mock_udef)
...
def mock_policy_dependencies(mocker):
    ...
    mocker.patch(
        'src.or_policies.single_vehicle.get_route_cost',
        return_value=50.0 
    )
...
def mock_lookahead_aux(mocker):
    ...
    mocker.patch(
        'src.or_policies.look_ahead.should_bin_be_collected',
        autospec=True,
        side_effect=lambda fill, rate: fill + rate >= 100
    )
    mocker.patch(
        'src.or_policies.look_ahead.update_fill_levels_after_first_collection',
        autospec=True,
        # Returns fill levels after collected bins are reset
        return_value=np.array([0.0, 10.0, 30.0, 40.0, 50.0]) 
    )
    mocker.patch(
        'src.or_policies.look_ahead.get_next_collection_day',
        autospec=True,
        return_value=5 # Mocked next overflow day
    )
    mocker.patch(
        'src.or_policies.look_ahead.add_bins_to_collect',
        autospec=True,
