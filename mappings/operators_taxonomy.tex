...
{Farthest Insertion} & Rosenkrantz et al. (1977) & Constructive / Repair & Spatial Diversification & Iteratively identifies the unassigned node geographically farthest from the current partial tour, inserting it at the locally optimal position. Naturally builds convex hulls first. & Requires $\mathcal{O}(|U| \times |V|)$ distance evaluations per step to identify the farthest unassigned node, but establishes robust macro-structures early. \\ \midrule
{Nearest Insertion} & Rosenkrantz et al. (1977) & Repair & Prox. Linking & Appends the geographically nearest unrouted customer directly to the end of the active partial tour. & Extremely fast $\mathcal{O}(n^2)$; inherently flawed for dense clustering due to "crossing-path" artifacts. \\ \midrule
