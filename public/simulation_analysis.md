...
**Figure 1:** *Scatter of all 30-day runs in the overflows–kg/km space, one panel per waste distribution. Colour encodes the mandatory selection variant, marker shape encodes the scenario (region/N), filled markers = FTSP, open markers = CLS. Dashed line = per-scenario Pareto front.*
...
**Figure 16:** *Scatter of all 90-day runs in the overflows–kg/km space, one panel per waste distribution. Colour encodes the mandatory selection variant, marker shape encodes the scenario (region/N), filled markers = FTSP, open markers = CLS. Dashed line = per-scenario Pareto front.*
...
**Table 7:** *Pareto-optimal policy configurations over the 90-day horizon — each unique (selection variant, constructor, improver) that appeared on the Pareto front of at least one scenario, sorted by scenario count; metrics averaged across those scenarios.*
...
**Table 8:** *Overflow counts by configuration over 90 days — min/max/mean across route constructors, per route improver.*
...
**Table 9:** *Route efficiency (kg/km) by configuration over 90 days — min/max/mean across route constructors, per route improver.*
...
**Table 10:** *Vehicle distance driven (km) by configuration over 90 days — min/max/mean across route constructors, per route improver.*
