...
def test_chart_worker_process_data():
    ...
    # Data Setup
    daily_data = {"PolicyA sample 1": {"metric1": {1: 10, 2: 20}}}
    historical_bin = {}
    latest_bin = {"PolicyA sample 1": {"bin_level": 50}}
    metrics = ["metric1"]
    data_mutex = QMutex()
...
def test_file_tailer_worker(mock_exists, mock_file):
    ...
    # Simpler approach: Just verify init and stop logic, and maybe simulate
    # one pass if possible. Given the blocking while loop, let's trust the logic
    # structure or run it in a separate thread for 0.1s in a real integration test.
