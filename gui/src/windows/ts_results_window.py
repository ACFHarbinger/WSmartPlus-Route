...
class SimulationResultsWindow(QWidget):
    ...
    def __init__(self, policy_names, log_path=None):
        ...
        self.summary_data = {"policies": [], "log": {}, "log_std": {}, "n_samples": 0}
    ...
    def _view_route_for_selected_day(self, target_key):
        ...
        depot_loc = next(
            ((p["lat"], p["lng"]) for p in points_to_plot if p.get("type") == "depot"),
    ...
    def _draw_selected_summary_heatmap(self):
        ...
        if not policy or not sample_str:
            return
    ...
    def redraw_summary_chart(self):
        ...
        if selection == "All Metrics":
            ...
            if selection not in udef.SIM_METRICS:
                return
    ...
    def setup_summary_area(self):
        ...
        ctl.addWidget(self.summary_metric_combo)
        ctl.addStretch()
        ...
        ctl2.addWidget(QLabel("Policy:"))
        ctl2.addWidget(self.summary_policy_combo)
        ctl2.addWidget(QLabel("Sample:"))
        ctl2.addWidget(self.summary_sample_combo)
        ctl2.addStretch()
