...
class TestLastMinutePolicies:
    ...
    def test_policy_last_minute_selects_bins(self, policy_deps):
        ...
        policy_deps["mocks"]["find_route"].assert_called_once()
        np.testing.assert_array_equal(policy_deps["mocks"]["find_route"].call_args[0][1], expected_to_collect)
    ...
    def test_policy_last_minute_and_path_expansion(self, mocker, mock_policy_common_data):
        ...
        mocker.patch("logic.src.policies.last_minute.find_route", return_value=[0, 2, 4, 0])
...
class TestAdvancedLookaheadPolicies:
    ...
    @pytest.mark.unit
    def test_hgs_limited_fleet(self, hgs_inputs):
        """Tests the custom HGS engine with limited fleet to verify split_limited fix."""
        (
            dist_matrix,
            demands,
            capacity,
        ...
        values = {
            "hgs_engine": "custom",
            "time_limit": 1,
            "max_vehicles": 1,  # Force limited split
        }
        routes, fitness, cost = run_hgs(
            dist_matrix,
            demands,
    ...
    def test_policy_lookahead_vrpp(self, mocker, mock_vrpp_inputs):
        ...
        mocker.patch("logic.src.policies.look_ahead.gp.Env", autospec=True)
    ...
    def test_policy_lookahead_sans(self, mocker, mock_vrpp_inputs):
        ...
        mocker.patch(
            "logic.src.policies.look_ahead.create_dataframe_from_matrix",
    ...
    def test_policy_lookahead_alns(self, mocker, mock_vrpp_inputs):
        ...
        mock_run_alns = mocker.patch(
            ...
            return_value=([[1, 2], [3]], 100.0, 150.0),  # routes, profit, cost
        )

        values = {"R": 1, "C": 1, "E": 1, "B": 10, "vehicle_capacity": 100}
    ...
    def test_policy_lookahead_alns_variants(self, mocker, mock_vrpp_inputs):
        ...
        # Mocks
        mock_run_alns = mocker.patch("logic.src.policies.look_ahead.run_alns", return_value=([], 0, 0))
        mock_run_alns_pkg = mocker.patch("logic.src.policies.look_ahead.run_alns_package", return_value=([], 0, 0))
        mock_run_alns_ort = mocker.patch("logic.src.policies.look_ahead.run_alns_ortools", return_value=([], 0, 0))
    ...
    def test_policy_lookahead_bcp(self, mocker, mock_vrpp_inputs):
        ...
        # --- Arrange ---
        mock_run_bcp = mocker.patch("logic.src.policies.look_ahead.run_bcp", return_value=([[1, 3], [2]], 200.0))
...
class TestGurobiOptimizer:
    ...
    def test_policy_gurobi_must_go_selection(self, mocker, mock_optimizer_data):
        ...
        mock_model.addVars.side_effect = lambda *args, **kwargs: MagicMock(__getitem__=lambda s, k: create_var_mock())
...
class TestHexalyOptimizer:
    ...
    def test_policy_hexaly_must_go_selection(self, mocker, mock_optimizer_data):
        ...
        mock_run_hexaly = mocker.patch.object(
            vrpp_opt_mod,
