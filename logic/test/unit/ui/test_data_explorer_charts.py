...
mock_st.selectbox.side_effect = lambda label, options, index=0, **kwargs: options[index] if index < len(options) else (options[0] if options else "")
mock_st.multiselect.side_effect = lambda label, options, default=None, **kwargs: default if default is not None else ([options[0]] if options else [])
...
def test_has_distribution_meta_and_unique_distributions():
    ...
    df2 = pd.DataFrame({
        "__Distributions__": ["d1", "d2", "d1", None]
    })
...
def test_render_visualization_tab():
    df = pd.DataFrame({
        "x": [1, 2, 3],
        "y": [10, 20, 30],
        "__Distributions__": ["d1", "d1", "d1"]
    })

    for chart_type in ["Line Chart", "Bar Chart", "Scatter Plot", "Area Chart", "Histogram", "Box Plot", "Heatmap", "Correlation Matrix"]:
        mock_st.reset_mock()
...
def test_find_td_table():
    tables = {
        "params (10x10)": pd.DataFrame({"x": [1]}),
        "coords": pd.DataFrame({"y": [2]})
    }
...
def test_render_td_coord_section():
    ...
    # Single Sample mode
    mock_st.reset_mock()
    mock_st.selectbox.side_effect = lambda label, options, index=0, **kwargs: "Single Sample" if label == "Display Mode" else "coords"
    _render_td_coord_section(tables, coord_keys, depot_keys)
    mock_st.plotly_chart.assert_called()

    # All Samples mode
    mock_st.reset_mock()
    mock_st.selectbox.side_effect = lambda label, options, index=0, **kwargs: "All Samples (up to 30)" if label == "Display Mode" else "coords"
    _render_td_coord_section(tables, coord_keys, depot_keys)
    mock_st.plotly_chart.assert_called()
...
def test_render_td_dist_section():
    tables = {
        "scalars (10x5)": pd.DataFrame([[1, 2], [3, 4]], columns=["node1", "node2"])
    }
    scalar_keys = ["scalars"]
