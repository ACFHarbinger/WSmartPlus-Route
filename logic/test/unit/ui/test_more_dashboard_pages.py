...
def test_render_algorithms(mock_registry):
    ...
    mock_registry.get_all.return_value = {DummyAlgo: {PolicyTag.CONSTRUCTION}}
...
def test_render_training_monitor(mock_overview, mock_controls, mock_load, mock_discover):
    ...
    with patch("logic.src.ui.pages.training.create_training_loss_chart"):
        render_training_monitor()
...
def test_render_all_metrics_table():
    runs_data = {"run1": pd.DataFrame({"epoch": [1, 2], "loss": [0.5, 0.4]})}
    with (
        patch("streamlit.multiselect", return_value=["epoch", "loss"]),
