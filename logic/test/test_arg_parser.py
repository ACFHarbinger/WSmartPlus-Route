...
class TestConfigsParser:
    ...
    def test_af_urange_default(self):
        ...
        args = ["script.py", "train", "--batch_size", "256", "--epoch_size", "128000"]
        with patch.object(sys, "argv", args):
            command, args_dict = parse_params()
...
class TestTrainCommand:
    ...
    def test_train_epoch_size_validation(self):
        ...
        args = [
            ...
            "256",
            "--epoch_size",
            "100000",  # Not divisible by 256
        ]
        with patch.object(sys, "argv", args):
            with pytest.raises(
                AssertionError,
                match="Epoch size must be integer multiple of batch size",
            ):
                parse_params()
...
class TestHPOptimCommand:
    ...
    def test_hp_optim_bayesian(self, base_hp_optim_args):
        ...
        args = base_hp_optim_args + ["--n_trials", "20", "--metric", "val_loss"]
        with patch.object(sys, "argv", args):
            command, args_dict = parse_params()
    ...
    def test_hp_optim_grid_search(self, base_hp_optim_args):
        ...
        args = base_hp_optim_args[:-1] + ["gs", "--grid", "0.0", "0.5", "1.0"]
        with patch.object(sys, "argv", args):
            command, args_dict = parse_params()
