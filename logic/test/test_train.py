...
class TestTrainFunctions:
    ...
    def test_train_reinforcement_learning_orchestration(self, mocker):
        ...
        mocker.patch(
            "logic.src.pipeline.train.setup_model_and_baseline",
            return_value=(mock_model, mock_baseline),
        )
        mocker.patch(
            "logic.src.pipeline.train.setup_optimizer_and_lr_scheduler",
...
class TestReinforce:
    ...
    def test_train_single_day(self, mocker, mock_train_model, mock_optimizer, mock_baseline):
        ...
        # Mocks
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.reinforce.tqdm",
            side_effect=lambda x, **kwargs: x,
        )
        mocker.patch("time.time", return_value=0.0)
        # Mock dependencies used by train_day
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.reinforce.prepare_batch",
            return_value={},
        )
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.base.complete_train_pass",
            return_value=None,
        )
        ...
        # mock_complete is not called in train_day/train_batch loop, only in post_day_processing
        # which is not called here. So we remove the assertion.
        pass
    ...
    def test_mrl_cb_update(self, mocker, mock_train_model, mock_optimizer, mock_baseline):
        ...
        mock_trainer_cls = mocker.patch(
            "logic.src.pipeline.reinforcement_learning.worker_train.ContextualBanditTrainer"
        )
...
class TestEpoch:
    ...
    def test_validate_update(self, mocker, mock_train_model):
        ...
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.epoch.prepare_batch",
            return_value={"input": torch.tensor([1])},
        )
        ...
        mock_train_model.return_value = (
            ...
            torch.zeros(1),  # ll
            {
                "overflows": torch.tensor([1.0]),
                "kg": torch.tensor([100.0]),
                "km": torch.tensor([10.0]),
...
class TestPostProcessing:
    ...
    def test_post_processing_loop(self, mocker, mock_train_model):
        ...
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.post_processing.DataLoader",
            return_value=dataset,
        )
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.post_processing.decode_routes",
            return_value=[[0, 1, 0]],
        )
        # Fix: return tensors to allow loss.backward()
        mocker.patch(
            "logic.src.pipeline.reinforcement_learning.core.post_processing.calculate_efficiency",
            return_value=(
        ...
        pp_model = post_processing.post_processing_optimization(mock_train_model, dataset, epochs=1)
