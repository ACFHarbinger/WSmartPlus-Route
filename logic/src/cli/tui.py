...
class TerminalUI:
    ...
    def display_header(self):
        ...
        self.console.print(
            Panel(
                header_text,
                subtitle="Interactive Command Center",
    ...
    def _prompt(
        ...
        try:
            if is_bool:
                ...
                result = prompt(
                    bool_prompt,
                    default="y" if default is True else "n" if default is False else "",
                    key_bindings=kb,
                )
    ...
    def configure_command(self, command: str) -> Optional[Dict[str, Any]]:
        ...
        if command == "test_sim":
            ...
            res = self._form_test_suite()
        else:
            self.console.print(
                f"[bold yellow]Warning:[/bold yellow] Form for '{command}' is not fully interactive yet."
            )
    ...
    def _form_test_sim(self) -> Optional[Dict[str, Any]]:
        ...
        val = self._prompt(
            "[bold green]Policies[/bold green] (space separated)",
            default="regular alns",
        )
        if val is None:
            return None
        ...
        opts["real_time_log"] = self._prompt(
            "[bold green]Enable real-time log window?[/bold green]",
            default=False,
            is_bool=True,
        )
        if opts["real_time_log"] is None:
            return None
    ...
    def _form_train(self) -> Optional[Dict[str, Any]]:
        ...
        opts = {}
        opts["model"] = self._prompt(
            "[bold green]Model type[/bold green]",
            default="am",
            choices=["am", "tam", "ddam"],
        )
        ...
        if opts["n_epochs"] is None:
            return None

        opts["train_time"] = self._prompt(
            "[bold green]Train over multiple days?[/bold green]",
            default=False,
            is_bool=True,
        )
        if opts["train_time"] is None:
            return None
    ...
    def _form_eval(self) -> Optional[Dict[str, Any]]:
        ...
        opts = {}
        opts["datasets"] = self._prompt(
            "[bold green]Datasets[/bold green] (space separated paths)",
            default="datasets/vrpp_20.pkl",
        )
        if opts["datasets"] is None:
            return None
        opts["datasets"] = opts["datasets"].split()

        opts["decode_type"] = self._prompt(
            "[bold green]Decode type[/bold green]",
            default="greedy",
            choices=["greedy", "sampling", "bs"],
        )
        if opts["decode_type"] is None:
            return None

        opts["eval_batch_size"] = self._prompt("[bold green]Eval batch size[/bold green]", default=256, is_int=True)
        if opts["eval_batch_size"] is None:
            return None
    ...
    def _form_gen_data(self) -> Optional[Dict[str, Any]]:
        ...
        opts = {}
        opts["problem"] = self._prompt(
            "[bold green]Problem type[/bold green]",
            default="vrpp",
            choices=["wcvrp", "vrpp", "cwcvrp"],
        )
        if opts["problem"] is None:
        ...
        if opts["dataset_size"] is None:
            return None

        opts["data_type"] = self._prompt(
            "[bold green]Data type[/bold green]",
            default="virtual",
            choices=["virtual", "real"],
        )
        if opts["data_type"] is None:
            return None
    ...
    def _form_file_system(self) -> Optional[Dict[str, Any]]:
        ...
        if sub_comm == "delete":
            ...
            if opts["test_checkpoint"] is None:
                return None

            opts["delete_preview"] = True
        return opts

    def _form_gui(self) -> Optional[Dict[str, Any]]:
        """Form for gui command."""
        opts = {}
        opts["app_style"] = self._prompt(
            "[bold green]Application Style[/bold green]",
            default="fusion",
            choices=["fusion", "windows", "macos"],
        )
        if opts["app_style"] is None:
            return None

        opts["test_only"] = self._prompt("[bold green]Run in test mode?[/bold green]", default=False, is_bool=True)
        if opts["test_only"] is None:
            return None

        return opts

    def _form_test_suite(self) -> Optional[Dict[str, Any]]:
        """Form for test_suite command."""
        opts = {}

        # Test selection
        module = self._prompt(
            "[bold green]Test Module[/bold green] (Leave empty for all)",
            default="",
        )
        if module is None:
            return None
        if module.strip():
            opts["module"] = module.split()

        opts["parallel"] = self._prompt("[bold green]Run in parallel?[/bold green]", default=False, is_bool=True)
        if opts["parallel"] is None:
            return None

        opts["verbose"] = self._prompt("[bold green]Verbose output?[/bold green]", default=True, is_bool=True)
    ...
    def run(self) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        while True:
            ...
            if self._prompt("\n[bold cyan]Run this command?[/bold cyan]", default=True, is_bool=True):
                ...
                res = self._prompt(
                    "Adjust parameters or select new command? (No to exit)",
                    default=True,
                    is_bool=True,
                )
