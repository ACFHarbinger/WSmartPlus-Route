...
def generate_swcvrp_data(dataset_size, scwvrp_size, waste_type, 
                       distribution="gamma1", area="Rio Maior", focus_graph=None, focus_size=0, method=None, num_days=1,
                       noise_mean=0.0, noise_variance=0.0):
    
    # 1. Generate Locations and Real Waste (Base Logic from WCVRP)
    if focus_graph is not None:
        assert focus_size > 0
        depot, loc, mm_arr, idx = load_focus_coords(scwvrp_size, method, area, waste_type, focus_graph, focus_size)
        remaining_coords_size = dataset_size - focus_size
        random_coords = np.random.uniform(mm_arr[0], mm_arr[1], size=(remaining_coords_size, scwvrp_size+1, mm_arr.shape[-1]))
        depots, locs = process_coordinates(random_coords, method, col_names=None)
        depot = np.concatenate((depot, depots))
        loc = np.concatenate((loc, locs))
        assert depot.shape[-1] == loc.shape[-1] and depot.shape[0] == loc.shape[0]
        if distribution == 'emp':
            data_dir = get_path_until_string(focus_graph, 'wsr_simulator')
            bins = Bins(scwvrp_size, data_dir, sample_dist=distribution, area=area, indices=idx[0], grid=None, waste_type=waste_type)
        else:
            bins = None
    else:
        bins = None
        coord_size = 2 if method != 'triple' else 3
        depot = np.random.uniform(size=(dataset_size, coord_size))
        loc = np.random.uniform(size=(dataset_size, scwvrp_size, coord_size))

    # Bin waste fill at the start (Real Waste)
    real_fill_values = []
    for _ in range(num_days):
        waste = generate_waste_prize(scwvrp_size, distribution, (depot, loc), dataset_size, bins)
        real_fill_values.append(waste)
    
    real_fill_values = np.transpose(np.array(real_fill_values), (1, 0, 2))
    
    # 2. Generate Noisy Waste
    # Noisy = Real + Noise
    # Ensure Non-Negative
    noise = np.random.normal(noise_mean, np.sqrt(noise_variance), real_fill_values.shape)
    noisy_fill_values = np.clip(real_fill_values + noise, 0, MAX_WASTE)

    # 3. Return (Depot, Loc, Real, Noisy, Max)
    return list(zip(
        depot.tolist(),
        loc.tolist(),
        real_fill_values.tolist() if real_fill_values.shape[0] > 1 else real_fill_values[0].tolist(),
        noisy_fill_values.tolist() if noisy_fill_values.shape[0] > 1 else noisy_fill_values[0].tolist(),
        np.full(dataset_size, MAX_WASTE).tolist()
    ))