...
def generate_waste(
    ...
    **kwargs: Any,
    ...
    if distribution == "empty":
        wp: Any = Constant(value=0.0).sample_array(size)
    elif distribution == "const":
        wp = Constant(value=1.0).sample_array(size)
    elif distribution in ["unif", "uniform"]:
        wp = Uniform().sample_array(size)
        ...
        try:
            ...
            gamma_option = 0
        wp = Gamma(option=gamma_option).sample_array(size)
