...
class StateSCWCVRP(NamedTuple):
    ...
    def initialize(input, edges, cost_weights=None, dist_matrix=None, visited_dtype=torch.uint8, hrl_mask=None):
        ...
        return StateSCWCVRP(
            ...
            noisy_waste=F.pad(noisy_waste, (1, 0), mode='constant', value=0),  # add 0 for depot
            real_waste=F.pad(real_waste, (1, 0), mode='constant', value=0),    # add 0 for depot
            ...
            cur_overflows=torch.sum((input['real_waste'] >= max_waste[:, None]), dim=-1), # Use Real waste for overflows
    ...
    def update(self, selected):
        ...
        prev_a = selected
        n_loc = self.noisy_waste.size(-1)  # Excludes depot
        ...
        selected_real_waste = self.real_waste.gather(1, selected).clamp(max=self.max_waste)
        cur_total_waste = self.cur_total_waste + selected_real_waste

        # Increase capacity if depot is not visited, otherwise set to 0
        used_capacity = (self.used_capacity + selected_real_waste) * (prev_a != 0).float()
