...
class SCWCVRP(BaseProblem):
    ...
    def get_costs(dataset, pi, cw_dict, dist_matrix=None):
        ...
        wastes = torch.cat(
            (torch.full_like(dataset["real_waste"][:, :1], -VEHICLE_CAPACITY), dataset["real_waste"]), 1
        )
    ...
    def __init__(
        ...
        if focus_graph is not None and focus_size > 0:
            ...
            if num_edges > 0 and edge_strat is not None:
                ...
                self.edges = None
            self.dist_matrix = torch.from_numpy(dist_matrix if 'dist_matrix_edges' not in locals() else dist_matrix_edges).float() / 100
            bins = Bins(size, os.path.join(os.getcwd(), "data", "wsr_simulator"), distribution, area=area, indices=idx[0], waste_type=waste_type, noise_mean=self.noise_mean, noise_variance=self.noise_variance) if distribution in ["gamma", "emp"] else None
        else:
        ...
        if filename is not None:
            with open(filename, "rb") as f:
                ...
                for item in tqdm(data[offset : offset + num_samples]):
                    ...
                    real_waste_t, noisy_waste_t = torch.FloatTensor(real_waste), torch.FloatTensor(noisy_waste)
                    if real_waste_t.dim() > 1:
                        real_waste_t = real_waste_t[0]
                    if noisy_waste_t.dim() > 1:
                        noisy_waste_t = noisy_waste_t[0]
                    instance = {"depot": torch.FloatTensor(depot), "loc": torch.FloatTensor(loc), "real_waste": real_waste_t, "noisy_waste": noisy_waste_t, "max_waste": torch.FloatTensor(max_waste)}
                    edges = calculate_edges(instance["loc"], num_edges, edge_strat)
                    if edges is not None:
                        instance["edges"] = edges
                    self.data.append(instance)
        else:
            self.data = [generate_instance(size, num_edges, edge_strat, distribution, bins, graph=(graph[0][i, :], graph[1][i, :, :]) if graph and i < focus_size else None, noise_mean=self.noise_mean, noise_variance=self.noise_variance) for i in tqdm(range(num_samples))]
