"""
Definition of the Stochastic Capacitated Waste Collection Vehicle Routing Problem (SCWCVRP).

This module defines the problem logic, cost calculations, and dataset handling
for the stochastic variant of the waste collection problem.
"""
...
class SCWCVRP(object):
    ...
    In this variant, bin levels are stochastic, and decisions are made based on
    noisy observations. The actual cost is calculated based on the real fill levels.
    """
    ...
    def get_costs(dataset, pi, cw_dict, dist_matrix=None):
        """
        Calculates the actual costs for a set of tours based on real waste levels.

        Args:
            dataset (dict): The dataset containing 'real_waste', 'max_waste', etc.
            pi (Tensor): The sequence of visited nodes (batch_size, tour_length).
            cw_dict (dict): Weights for different cost components (e.g., overflows, length).
            dist_matrix (Tensor, optional): Precomputed distance matrix.

        Returns:
            tuple: (total_cost, cost_breakdown_dict, None)
        """
        ...
        if pi.size(-1) == 1:
            ...
            overflows = torch.sum(
                dataset["real_waste"] >= dataset["max_waste"][:, None], dim=-1
            )
        ...
        overflows = torch.sum(
            current_node_wastes[:, 1:] >= dataset["max_waste"][:, None], dim=-1
        )
        ...
        waste = dataset["real_waste"].sum(dim=-1) - current_node_wastes[:, 1:].sum(
            dim=-1
        )
    ...
    def make_dataset(*args, **kwargs):
        """
        Creates an instance of the SWCVRPDataset.

        Returns:
            SWCVRPDataset: The initialized dataset.
        """
    ...
    def make_state(*args, **kwargs):
        """
        Creates an initial state for the SCWCVRP.
    ...
    def beam_search(
        ...
        """
        Performs beam search to find high-quality solutions.
...
def generate_instance(
    ...
    *args,
    ...
    """
    Generates a single instance of the SCWCVRP.

    Args:
        size (int): Number of nodes in the graph.
        edge_threshold (float): Threshold or k for edge generation.
        edge_strategy (str): Strategy for edge generation ('dist' or 'knn').
        distribution (str): Distribution for waste generation.
        bins (Bins): Bins object for waste simulation.
        graph (tuple, optional): (depot, loc) coordinates if fixed.
        noise_mean (float): Mean of the Gaussian noise added to real waste.
        noise_variance (float): Variance of the Gaussian noise.

    Returns:
        dict: A dictionary containing instance data (loc, depot, real_waste, noisy_waste, max_waste, edges).
    """
    ...
    if noise_variance > 0:
        noise = torch.normal(
            mean=noise_mean, std=noise_variance**0.5, size=real_waste.size()
        )
...
class SWCVRPDataset(Dataset):
    ...
    Supports loading from pickle files or generating instances on the fly with noise.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            size (int): Graph size (number of locations).
            num_samples (int): Number of instances to load or generate.
            offset (int): Offset for loading from file.
            distribution (str): Waste distribution type.
            area (str): Geographic area for coordinates.
            vertex_strat (str): Strategy for vertex placement.
            number_edges (int): Number of edges or threshold for graph structure.
            edge_strat (str): Strategy for edge selection.
            focus_graph (str, optional): Name of a specific graph to focus on.
            focus_size (int): Number of instances to focus on the fixed graph.
            dist_strat (str, optional): Strategy for distance calculation.
            waste_type (str, optional): Type of waste.
            dist_matrix_path (str, optional): Path to precomputed distance matrix.
            noise_mean (float): Mean of noise for stochasticity.
            noise_variance (float): Variance of noise.
        """
                ...
                for item in tqdm(data[offset : offset + num_samples]):
                    ...
                    if num_edges > 0:
                        distance_matrix = squareform(
                            pdist(instance["loc"], metric="euclidean")
                        )
                        if edge_strat == "dist":
                            instance["edges"] = torch.tensor(
                                get_edge_idx_dist(distance_matrix, num_edges)
                            ).to(dtype=torch.long)
                        elif edge_strat == "knn":
                            neg_adj_matrix = get_adj_knn(distance_matrix, num_edges)
                            instance["edges"] = torch.tensor(
                                adj_to_idx(neg_adj_matrix)
                            ).to(dtype=torch.long)
    ...
    def __getitem__(self, idx):
        """Returns a single instance from the dataset at the given index."""
