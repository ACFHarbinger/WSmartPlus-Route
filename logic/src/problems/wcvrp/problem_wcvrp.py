"""
Definition of the Waste Collection Vehicle Routing Problem (WCVRP) and its variants.

This module defines the problem logic, cost calculations, and dataset handling
for waste collection scenarios, including basic WCVRP, Capacitated WCVRP (CWCVRP),
and Split-Delivery WCVRP (SDWCVRP).
"""
...
class WCVRP(object):
    ...
    def make_dataset(*args, **kwargs):
        """
        Creates an instance of the WCVRPDataset.

        Returns:
            WCVRPDataset: The initialized dataset.
        """
    ...
    def make_state(*args, **kwargs):
        """
        Creates an initial state for the WCVRP.

        Returns:
            StateWCVRP: The initial state.
        """
    ...
    def beam_search(
        ...
        """
        Performs beam search to find high-quality solutions for WCVRP.
...
class CWCVRP(object):
    ...
    Extends WCVRP by adding vehicle capacity constraints.
    """
    ...
    def get_costs(dataset, pi, cw_dict, dist_matrix=None):
        """
        Calculates the total cost for a set of tours, validating vehicle capacity.
        ...
        if pi.size(-1) == 1:
            ...
            cost = (
                overflows if cw_dict is None else cw_dict["overflows"] * overflows
            )  # length and waste are 0
            c_dict = {
                "overflows": overflows,
    ...
    def make_state(*args, **kwargs):
        """
        Creates an initial state for the CWCVRP.

        Returns:
            StateCWCVRP: The initial state.
        """
        kwargs.pop("profit_vars", None)
    ...
    def beam_search(
        ...
        """
        Performs beam search for CWCVRP.

        Args:
            input (dict): The input data instance.
            beam_size (int): The number of beams to maintain.
            cost_weights (dict): Weights for the cost function.
            edges (Tensor, optional): Edge information.
            expand_size (int, optional): Number of expansions per beam.
            compress_mask (bool): Whether to compress the mask for efficiency.
            model (nn.Module, optional): The model used for node selection.
            max_calc_batch_size (int): Maximum batch size for calculations.
...
class SDWCVRP(object):
    ...
    In this variant, a bin's waste can be collected over multiple visits.
    """
    ...
    def get_costs(dataset, pi, cw_dict, dist_matrix=None):
        """
        Calculates the total cost for split delivery tours.
    ...
    def make_state(*args, **kwargs):
        """
        Creates an initial state for the SDWCVRP.

        Returns:
            StateSDWCVRP: The initial state.
        """
        kwargs.pop("profit_vars", None)
    ...
    def beam_search(
        ...
        Args:
            ...
            edges (Tensor, optional): Edge information.
            expand_size (int, optional): Number of expansions per beam.
            compress_mask (bool): Whether to compress the mask. MUST BE FALSE.
            model (nn.Module, optional): The model used for node selection.
            max_calc_batch_size (int): Maximum batch size for calculations.
...
def make_instance(edge_threshold, edge_strategy, args):
    """
    Creates a single WCVRP instance from raw data.
...
def generate_instance(
    size, edge_threshold, edge_strategy, distribution, bins, *args, graph=None
):
    """
    Gathers or generates a single random WCVRP instance.
...
class WCVRPDataset(Dataset):
    ...
    def __init__(
        ...
        if filename is not None:
            ...
            self.data = [
                (
                    generate_instance(size, num_edges, edge_strat, dist, bins, args)
                    if i >= focus_size
