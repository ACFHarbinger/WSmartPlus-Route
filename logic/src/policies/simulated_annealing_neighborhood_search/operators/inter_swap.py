...
def swap_2_routes(routes_list: List[List[int]], rng: Random) -> None:
    """
    ...
    if len(routes_list) > 1:
        chosen_routes = rng.sample(routes_list, 2)
        route1, route2 = chosen_routes

        if len(route1) > 2 and len(route2) > 2:
            bin1 = rng.sample(route1[1 : len(route1) - 1], 1)[0]
            bin2 = rng.sample(route2[1 : len(route2) - 1], 1)[0]
...
def swap_n_2_routes_random(routes_list: List[List[int]], rng: Random, n: Optional[int] = None) -> Optional[int]:
    """
    ...
    Args:
        routes_list (List[List[int]]): Current routing solution.
        rng (Random): Random number generator.
        n (int, optional): Number of bins to swap.
...
def swap_n_2_routes_consecutive(routes_list: List[List[int]], rng: Random, n: Optional[int] = None) -> Optional[int]:
    """
    ...
    if len(routes_list) > 1:
        ...
        if len(route1) >= chosen_n + 2 and len(route2) >= chosen_n + 2:
            ...
            start1 = rng.randint(1, len(route1) - chosen_n)
            start2 = rng.randint(1, len(route2) - chosen_n)
