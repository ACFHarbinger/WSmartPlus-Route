...
def uncross_arcs_in_routes(previous_solution, p_vehicle, p_load, p_route_difference, p_shift, data, points, distance_matrix, values):
    ...
    for k in previous_solution:
        ...
        globals()['relevant_crossed_arcs_route_{0}'.format(index_route)] = deepcopy(globals()['crossed_arcs_route_{0}'.format(index_route)])
    ...
    profit_after_uncross = compute_profit(solution_after_uncross,p_vehicle,p_load,p_route_difference,p_shift,data,distance_matrix, values)
