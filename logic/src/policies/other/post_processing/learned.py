...
Training is performed offline by tools/train_learned_postprocessor.py. The
post-processor loads the trained model at init time and uses it read-only
during inference.
"""
...
@PostProcessorRegistry.register("learned")
class LearnedPostProcessor(IPostProcessor):
    """
    Learned post-processor using a pre-trained move scorer.
    ...
    DEFAULT_WEIGHTS_PATH = Path("assets") / "model_weights" / "learned_postprocessor.pt"
