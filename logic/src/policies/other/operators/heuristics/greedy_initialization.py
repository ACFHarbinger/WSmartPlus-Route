...
def _prune_unprofitable_routes(
    ...
    final_routes = []
    for route in routes:
        if any(n in mandatory_nodes_set for n in route):
...
def _greedy_profit_insertion(
    ...
    return _prune_unprofitable_routes(routes, dist_matrix, wastes, R, C, mandatory_nodes_set)
