...
from logic.src.policies.other.branching_solvers.pricing.labels import Label
    ...
    def _extend_label(
        ...
        for subset, dual in rcc_duals.items():
            if (label.node in subset) != (next_node in subset):
                # If you add node duals in Farkas, you should probably add cut duals too
                rc_delta += dual if self.is_farkas else -dual

        if edge_clique_duals:
            can_edge = tuple(sorted((label.node, next_node)))
            rc_delta -= edge_clique_duals.get(can_edge, 0.0)
