...
@MustGoSelectionRegistry.register("whittle")
class WhittleIndexSelection(IMustGoSelectionStrategy):
    """
    ...
    def select_bins(self, context: SelectionContext) -> List[int]:
        ...
        if context.n_vehicles <= 0:
            must_go = np.nonzero(whittle_indices > 1e-3)[0]
        else:
            ...
            must_go = top_k[whittle_indices[top_k] > 1e-3]

        return sorted((must_go + 1).tolist())
