...
Example:
    >>> from logic.src.policies.other.must_go.selection_rollout import RolloutSelection
...
@MustGoSelectionRegistry.register("rollout")
class RolloutSelection(IMustGoSelectionStrategy):
    """
    ...
    def select_bins(self, context: SelectionContext) -> List[int]:
        ...
        from logic.src.policies.other.must_go.base.selection_factory import MustGoSelectionFactory
        ...
        try:
            base_policy = MustGoSelectionFactory.create_strategy(base_policy_name)
        except Exception:
        ...
        for i in range(n_bins):
            ...
            if r_collect > r_defer:
                must_go_indices.append(i)

        return sorted((np.array(must_go_indices) + 1).tolist())
    ...
    def _eval_bin(
        ...
        base_policy: IMustGoSelectionStrategy,
        horizon: int,
        n_scenarios: int,
        bin_cap: float,
        revenue_kg: float,
