...
Attributes:
-----------
LookaheadTables: Container for all lookahead-derived quantities.
LookaheadValuator: Class for building the scenario tree and producing the V / rho / early_regret tables.
lagrangian_corrected_prizes: Produce effective prize coefficients for one Lagrangian subproblem.
...
class LookaheadValuator:
    ...
    _tree : Optional[ScenarioTree]
        The scenario tree.
    """

    def __init__(self, params: LookaheadParams):
        """
        Initialize the LookaheadValuator.
    ...
    def get_scenario_tree(self) -> ScenarioTree:
        """Return the scenario tree generated by the last call to compute().
    ...
    def _extract_expected_path(self, tree: ScenarioTree, N: int) -> np.ndarray:
        ...
        Returns:
        ----------
        np.ndarray : (N, T+1) expected fill levels.
        """
        T = tree.horizon
    ...
    def _compute_value_table(self, expected_fill: np.ndarray) -> np.ndarray:
        ...
        Args:
        ----------
        expected_fill : (N, T+1) expected fill levels.

        Returns:
        ----------
        np.ndarray : (N, T) value table.
        """
    ...
    def _compute_regret_table(self, V: np.ndarray) -> np.ndarray:
        """
        Compute regret for each bin at each time period.
...
def lagrangian_corrected_prizes(
    ...
    Returns:
    ----------
    np.ndarray : (N, T) effective prize coefficients.
    """
    if side == "knapsack":
