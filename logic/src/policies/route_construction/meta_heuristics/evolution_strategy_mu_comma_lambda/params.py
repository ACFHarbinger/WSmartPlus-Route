...
Attributes:
    MuCommaLambdaESParams: Parameter dataclass for (μ,λ)-ES.

Example:
    >>> params = MuCommaLambdaESParams(mu=15, lambda_=100)
"""
...
class MuCommaLambdaESParams:
    ...
    Attributes:
        ...
        lambda_: The number of offspring generated in each generation (λ).
            Standard (μ,λ) theory suggests that λ should be significantly
            larger than μ (e.g., λ ≈ 7μ) to ensure efficient step-size
            self-adaptation.

        n_removal: The mutation strength parameter. Defines the number
        ...
        max_iterations: The generational loop limit. Serves as a
            primary termination criterion for the search process.

        local_search_iterations: The intensity of the local optimization
            applied to each offspring. This governs the fine-tuning of
            candidate solutions post-mutation.

        time_limit: Wall-clock duration in seconds. The algorithm
    ...
    def from_config(cls, config: Any) -> MuCommaLambdaESParams:
        ...
        Returns:
            MuCommaLambdaESParams instance.
        """
