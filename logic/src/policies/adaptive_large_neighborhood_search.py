...
class ALNSSolver:
    ...
    def solve(self, initial_solution: Optional[List[List[int]]] = None) -> Tuple[List[List[int]], float, float]:
        ...
        for route in best_routes:
            for node_idx in route:
                collected_revenue += self.demands.get(node_idx, 0) * self.R
    ...
    def cluster_removal(self, routes: List[List[int]], n_remove: int) -> Tuple[List[List[int]], List[int]]:
        ...
        if not routes[seed_route_idx]:
            return self.random_removal(routes, n_remove)
        ...
        for node in removed_nodes:
            ...
            for r_idx, route in enumerate(routes):
                ...
                for i in range(len(route) + 1):
                    ...
                    cost_increase = (
                        self.dist_matrix[prev][node] + self.dist_matrix[node][nex] - self.dist_matrix[prev][nex]
                    )
    ...
    def regret_2_insertion(self, routes: List[List[int]], removed_nodes: List[int]) -> List[List[int]]:
        ...
        while pending:
            ...
            for node in pending:
                ...
                best = valid_moves[0]
                second = (
                    valid_moves[1] if len(valid_moves) > 1 else (best[0] * 1.5, -1, -1)
                )  # infinite regret surrogate
...
def run_alns_ortools(dist_matrix, demands, capacity, R, C, values):
    ...
    if solution:
        ...
        for route in routes:
            for node_idx in route:
                collected_revenue += demands.get(node_idx, 0) * R
