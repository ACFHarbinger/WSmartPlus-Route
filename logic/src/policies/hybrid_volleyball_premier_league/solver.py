"""
Hybrid Volleyball Premier League (HVPL) algorithm combining ACO, VPL, and ALNS.

Integrates three algorithmic paradigms in sequential phases:
    Phase 1: ACO - Intelligent population initialization with pheromone guidance
    Phase 2: VPL + HGS - Population evolution with genetic operators
    Phase 3: ALNS - Deep local search refinement per solution

The hybrid design addresses the original VPL's convergence issues by:
    - ACO: High-quality diverse initialization
    - HGS: Dynamic genetic operators replacing deterministic coaching
    - ALNS: Intensive local refinement for exploitation
...
class HVPLSolver:
    ...
    def solve(self) -> Tuple[List[List[int]], float, float]:
        ...
        # ═══════════════════════════════════════════════════════════
        # PHASE 2: VPL EVOLUTION WITH HGS
        # ═══════════════════════════════════════════════════════════
    ...
    def _aco_initialization(self) -> List[List[List[int]]]:
        ...
        while len(population) < self.params.n_teams:
    ...
    def _selection(
        ...
        # Select top N
        selected_indices = sorted_indices[: self.params.n_teams]
    ...
    def _substitution_phase(
        ...
        for i in range(self.params.n_teams - n_substitute, self.params.n_teams):
