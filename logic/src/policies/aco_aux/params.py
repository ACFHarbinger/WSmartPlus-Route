...
class ACOParams:
    ...
    rho: float = 0.1
    q0: float = 0.9
    tau_0: Optional[float] = None  # Computed from initial solution if None
    tau_min: float = 0.001
    tau_max: float = 10.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.k_sparse < 1:
            raise ValueError(f"k_sparse must be >= 1, got {self.k_sparse}")
        if self.n_ants < 1:
            raise ValueError(f"n_ants must be >= 1, got {self.n_ants}")
