...
def hyperparameter_optimization(opts):
    ...
    """
    if opts["cpu_cores"] > 1:
        update_lock_wait_time(opts["cpu_cores"])
    ...
    try:
        ...
        raise Exception(
            "directories to save hyper-parameter optimization output files do not exist and could not be created"
        )

    # Save the optimization configuration
    with open(
        os.path.join(opts["save_dir"], "optimization_results", "optimization_config.json"),
        "w",
    ) as f:
...
def train_reinforcement_learning(opts, train_function, cost_weights=None):
    ...
    torch.save(
        ...
        os.path.join(
            ROOT_DIR,
            opts["final_dir"],
            "epoch-{}.pt".format(opts["epoch_start"] + opts["n_epochs"] - 1),
        ),
    )
