...
class WeightContextualBandit(WeightAdjustmentStrategy):
    ...
    def propose_weights(self, context=None):
        ...
        if context is None:
            ...
            # Unlike RewardWeightOptimizer which used internal history, this one seems to need
            # 'dataset' for context. If context is not passed, we might be in trouble unless we
            # stored it from previous steps or it's optional. Looking at original code
            # `get_current_weights` took `dataset`. We will assume context['dataset'] or similar
            # is available or context IS the dataset features already processed.
    ...
    def _generate_weight_configs(self, initial_weights, num_configs):
        ...
        if self.weight_ranges:
            ...
            while len(configs) < num_configs:
                config = {
                    component: random.uniform(
                        self.weight_ranges[component][0],
    ...
    def _get_context_features(self, dataset, init_value=0.5):
        ...
        for fkey in self.context_features:
            ...
            elif fkey == "length" and "length" not in context:
                context = _set_context_value("length", init_value, context)  # Normalized value
            elif fkey == "visited_ratio" and "visited_ratio" not in context:
            ...
            for i in range(self.num_configs):
                ...
                if len(rewards) > 0:
                    ...
                    confidence = self.exploration_factor * math.sqrt(
                        math.log(self.total_trials) / max(1, self.trials[i])
                    )
    ...
    def _select_thompson_sampling(self, context_key):
        # If we have context-specific data, use it
        if context_key in self.context_rewards and any(
            len(self.context_rewards[context_key][i]) > 0 for i in range(self.num_configs)
        ):
    ...
    def update(self, reward, cost_components, context=None, epsilon_params=(0.01, 1.0)):
        """
        Update the bandit with the observed reward.
        """
        # Get context key
        if context is None and self.contexts:
        ...
        if context_key:
            ...
            if len(self.context_rewards[context_key][self.current_config_idx]) > self.window_size:
                self.context_rewards[context_key][self.current_config_idx] = self.context_rewards[context_key][
                    self.current_config_idx
                ][-self.window_size :]
