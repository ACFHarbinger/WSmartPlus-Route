...
class ParetoSolution:
    ...
    def dominates(self, other):
        ...
        strictly_better = (self.objectives["waste_efficiency"] > other.objectives["waste_efficiency"]) or (
...
class ParetoFront:
    ...
    def _prune_with_clustering(self):
        ...
        objectives = np.array(
            [[s.objectives["waste_efficiency"], s.objectives["overflow_rate"]] for s in self.solutions]
        )
    ...
    def get_reference_points(self, num_points=5):
        ...
        sorted_solutions = sorted(self.solutions, key=lambda s: s.objectives["waste_efficiency"])
    ...
    def plot_front(self, save_path=None):
        ...
        for i, sol in enumerate(self.solutions):
            if i % max(1, len(self.solutions) // 5) == 0:  # Label only some points to avoid crowding
                plt.annotate(
                    f"w_waste={sol.weights['w_waste']:.1f}\nw_over={sol.weights['w_over']:.1f}",
