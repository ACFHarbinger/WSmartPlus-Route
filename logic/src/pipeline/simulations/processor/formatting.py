...
def format_coordinates(coords: Any, method: str, col_names: Optional[list[str]] = None) -> Tuple[Any, Any]:
    ...
    if method == "c3d":
        depot, loc = _format_c3d(lat, lng, IS_PANDAS)
    elif method == "s4d":
        depot, loc = _format_s4d(lat, lng, IS_PANDAS)
    elif method == "mun":
        lat, lng, coords = _format_mun(lat, lng, coords, IS_PANDAS)
    elif method == "smsd":
        lat, lng, coords = _format_smsd(lat, lng, coords, IS_PANDAS)
    elif method == "ecp":
        lat, lng = _format_ecp(lat, lng, coords, IS_PANDAS)
    elif method == "utmp":
