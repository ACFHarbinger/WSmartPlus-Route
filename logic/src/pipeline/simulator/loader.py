        ...
        try:
            if src_area == 'mixrmbac':
                ...
                if number_of_bins == 104:
                    df = pd.read_csv(os.path.join(d_dir, 'bins_waste', f"Rio_Maior_Sensores_2021_2024_cleaned_104.csv"))
                    ...
                    data = data.round()
                    data.columns = data.columns.astype(int)
                    coords_tmp = pd.read_csv(os.path.join(d_dir, 'coordinates', f'coordinates104.csv'))
                    coords_tmp = coords_tmp.rename(columns={'Lon': 'Lng', 'Longitude': 'Lng', 'Latitude': 'Lat'})
                else:
                    data = _preprocess_county_date(pd.read_csv(os.path.join(d_dir, 'bins_waste', f"old_out_crude_rate[{src_area}].csv")))
                ...
                coords_tmp = pd.read_csv(os.path.join(d_dir, 'coordinates', f'out_info[figdafoz].csv'))
                coords_tmp = coords_tmp.rename(columns={'Latitude': 'Lat', 'Longitude': 'Lng'})
                if wtype: coords_tmp = coords_tmp[coords_tmp['Tipo de Residuos'] == wtype]
                bins_coordinates = coords_tmp[['ID', 'Lat', 'Lng']]
                data = _preprocess_county_data(data)
