...
from .loader import load_depot, load_simulator_data, load_area_and_waste_type_params
from .network import compute_distance_matrix, apply_edges, get_paths_between_states, haversine_distance
from logic.src.utils.definitions import EARTH_RADIUS, MAX_WASTE, EARTH_WMP_RADIUS
...
class SimulationDataMapper:
    ...
    def format_coordinates(self, coords: Any, method: str, col_names: Optional[List[str]] = ['Lat', 'Lng']) -> Tuple[Any, Any]:
        """
        ...
        if method == 'c3d': # Conversion to 3D Cartesian coordinates
            ...
            if method == 'mun': # Mean (μ) normalization
                ...
                if IS_PANDAS:
                    lat = (lat - lat.mean())/(lat.max() - lat.min()) if lat.max() != lat.min() else lat # Careful with div by zero?
...
def create_dataframe_from_matrix(matrix):
    ...
    data = pd.DataFrame({
        '#bin': ids_rota,
        'Stock': enchimentos,
        'Accum_Rate': np.zeros(len(ids_rota))
    })
    return data
