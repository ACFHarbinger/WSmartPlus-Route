...
class ConfigsParser(argparse.ArgumentParser):
    ...
    def _str_to_nargs(self, nargs):
        ...
        Args:
            nargs: The argument value(s).

        Returns:
            list or Any: The processed argument value.
        """
    ...
    def parse_command(self, args=None):
        ...
        Args:
            args (list, optional): List of argument strings. Defaults to sys.argv[1:].
    ...
    def parse_process_args(self, args=None, command=None):
        ...
        Args:
            args (list, optional): List of argument strings. Defaults to sys.argv[1:].
            command (str, optional): Pre-determined command name.

        Returns:
            tuple: (command, opts_dict)
        """
        if args is None:
            args = sys.argv[1:]
...
def add_train_args(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["trajectory", "distributions", "embeddings", "heatmaps", "loss"],
        choices=[
            "trajectory",
            "distributions",
            "embeddings",
...
def add_gen_data_args(parser):
    ...
    parser.add_argument(
        "--data_dir", default="datasets", help="Create datasets in data"
    )
    parser.add_argument(
        "--problem",
        type=str,
    ...
    parser.add_argument(
        "--sigma", type=float, nargs="+", default=0.6, help="Variance of Gaussian noise"
    )
    parser.add_argument(
        "--data_distributions",
        nargs="+",
    ...
    parser.add_argument(
        "--dataset_size", type=int, default=128_000, help="Size of the dataset"
    )
    parser.add_argument(
        "--graph_sizes",
        type=int,
    ...
    parser.add_argument(
        "--penalty_factor", type=float, default=3.0, help="Penalty factor for problems"
    )
    parser.add_argument(
...
def get_main_parser():
    ...
    Returns:
        ConfigsParser: The configured main argument parser.
    """
    ...
    subparsers = parser.add_subparsers(
        help="Main command", dest="command", required=True
    )

    # 1. Train Model (Base Training)
    train_parser = subparsers.add_parser(
        "train", help="Train reinforcement learning model"
    )
    add_train_args(train_parser)

    # 2. Meta RL Training
    mrl_parser = subparsers.add_parser(
        "mrl_train", help="Train meta-reinforcement learning model"
    )
    add_mrl_train_args(mrl_parser)

    # 3. Hyperparameter Optimization
    hp_optim_parser = subparsers.add_parser(
        "hp_optim", help="Hyperparameter optimization"
    )
    add_hp_optim_args(hp_optim_parser)

    # 4. Generate Data
    gen_data_parser = subparsers.add_parser(
        "gen_data", help="Generate problem datasets"
    )
    ...
    # 6. Test Simulator
    test_sim_parser = subparsers.add_parser(
        "test_sim", help="Run WSR simulator test policies"
    )
    add_test_sim_args(test_sim_parser)

    # 7. File System Tools (Requires inner sub-parsers)
    files_parser = subparsers.add_parser(
        "file_system", help="File system manipulation tools"
    )
    add_files_args(files_parser)

    # 8. Test Suite
    test_suite_parser = subparsers.add_parser(
        "test_suite", help="Run program test suite"
    )
...
def validate_gen_data_args(args):
    ...
    if args["problem"] in ["all", "swcvrp"]:
        assert (
            "mu" in args and args["mu"] is not None
        ), "Must specify mu when generating swcvrp datasets"
        assert (
            "sigma" in args and args["sigma"] is not None
        ), "Must specify sigma when generating swcvrp datasets"
        assert len(args["mu"]) == len(
            args["sigma"]
        ), "Must specify same number of mu and sigma values"
