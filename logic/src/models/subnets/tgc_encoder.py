...
class GraphConvolutionLayer(nn.Module):
    ...
    """
    def __init__(self, embed_dim, feed_forward_hidden, agg, normalization, 
                epsilon_alpha, learn_affine, track_stats, mbeta, lr_k, n_groups,
                activation, af_param, threshold, replacement_value, n_params, uniform_range):
        """Initializes the GraphConvolutionLayer."""
        super(GraphConvolutionLayer, self).__init__()
        self.ff_conv = SkipConnection(
            FFConvSubLayer(embed_dim, feed_forward_hidden, agg, activation, 
                        af_param, threshold, replacement_value, n_params, uniform_range)
        )
        self.norm = Normalization(embed_dim, normalization, epsilon_alpha, 
                                learn_affine, track_stats, mbeta, n_groups, lr_k)
...
class TransGraphConvEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        layers = [
            ...
            GraphConvolutionLayer(embed_dim, feed_forward_hidden, agg, normalization, 
                                epsilon_alpha, learn_affine, track_stats, momentum_beta, 
                                locresp_k, n_groups, activation, af_param, threshold, 
                                replacement_value, n_params, uniform_range) for _ in range(n_sublayers)
        ]
