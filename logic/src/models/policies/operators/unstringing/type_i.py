...
This module provides a GPU-accelerated implementation of the Type I unstringing
...
Attributes:
    vectorized_type_i_unstringing: Vectorized Type I Unstringing local search.

Example:
    >>> tours = torch.tensor([[0, 1, 2, 3, 4, 5, 0]])
    >>> dist = torch.randn(1, 6, 6)
    >>> opt_tours = vectorized_type_i_unstringing(tours, dist)
"""
...
def vectorized_type_i_unstringing(
    ...
    """Vectorized Type I Unstringing local search across a batch of tours.
...
def _evaluate_type_i_move(tour: torch.Tensor, dist: torch.Tensor, i: int, j: int, k: int, N: int) -> float:
