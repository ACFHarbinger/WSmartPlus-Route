...
def _plot_line_plus_ref(ax, sub_line: pd.DataFrame, sub_ref: pd.DataFrame, metric: str, dist: str,
                        label: str | None = None, ls: str = "-", alpha: float = 1.0) -> None:
    """Rio Maior evolution line plus the Figueira da Foz N=350 reference diamond."""
    color = DIST_COLORS.get(dist, "gray")
    if len(sub_line):
        ax.plot(sub_line["N"].values, sub_line[metric].values, ls, marker="o",
                color=color, linewidth=2, markersize=8, alpha=alpha,
                label=label or DIST_LABELS.get(dist, dist))
    if len(sub_ref):
        ax.scatter(sub_ref["N"].values, sub_ref[metric].values, marker="D", s=110,
                   color=color, edgecolors=CITY_COLORS.get(REF_CITY, "#e08030"),
                   linewidths=1.8, alpha=alpha, zorder=4,
        ...
        for di, dist in enumerate(dists):
            ...
            for ci, city in enumerate(cities):
                ...
                if len(vals):
                    ...
                    ax.bar(ci + off, np.mean(vals), 0.35, color=DIST_COLORS.get(dist, "gray"),
                           alpha=0.85, label=DIST_LABELS.get(dist, dist) if ci == 0 else "")
...
def gen_npz_size_scaling(npz: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig.suptitle("NPZ Statistics vs Network Size (lines: Rio Maior · diamonds: FFZ-350)",
...
def gen_npz_horizon_comparison(npz: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle(
        f"{' vs '.join(str(h) + '-day' for h in horizons)} Horizon Comparison "
        "(lines: Rio Maior · diamonds: FFZ-350)",
        fontsize=14,
        fontweight="bold",
    )
    for ax, (metric, ylabel) in zip(axes, [
        ...
        for dist in sorted(npz["dist"].unique()):
            for horizon, ls in zip(horizons, ["-", "--", ":"], strict=False):
                ...
                _plot_line_plus_ref(
                    ax, rm, ref, metric, dist,
                    label=f"{DIST_LABELS.get(dist, dist)} {horizon}d",
                    ls=ls, alpha=1.0 if horizon == horizons[0] else 0.7,
                )
        ax.set_xlabel("Network size N", fontsize=10)
        ax.set_ylabel(ylabel, fontsize=10)
        ax.set_title(metric, fontsize=11)
...
def gen_npz_city_comparison(npz: pd.DataFrame, ext: pd.DataFrame, out_dir: Path) -> None:
        ...
        for dist in sorted(sub30["dist"].unique()):
            ...
            for city in cities:
                ...
                for _, row in sub_cd.iterrows():
                    ...
                    ax.scatter(row["N"], row[metric], c=CITY_COLORS.get(city, "#a0a0a0"),
                               marker=marker, s=150, alpha=0.85, edgecolors=ec, linewidths=1.5)
        ax.set_xlabel("Network size N", fontsize=10)
        ax.set_ylabel(ylabel, fontsize=10)
        ax.set_title(metric, fontsize=11)
        ax.yaxis.grid(True, alpha=0.4)
    ...
    dist_patches = [
        ...
        plt.scatter([], [], c="gray", s=60, marker="o", edgecolors="white", linewidths=1.5,
...
def gen_npz_td_alignment(npz: pd.DataFrame, td: pd.DataFrame, out_dir: Path) -> None:
    ...
    for dist in sorted(sub30["dist"].unique()):
        ...
        ax.plot(rm_sub["N"].values, rm_sub["mean_kg"].values, "o-", color=color,
                linewidth=2, markersize=8, label=f"NPZ {dist_label} (RM)")
        if len(ref_sub):
            ax.scatter(ref_sub["N"].values, ref_sub["mean_kg"].values, marker="D", s=110,
                       color=color, edgecolors=CITY_COLORS.get(REF_CITY, "#e08030"), linewidths=1.8,
                       zorder=4, label=f"NPZ {dist_label} (FFZ-350)")
        if len(td_sub):
            ax.plot(td_sub["N"].values, td_sub["waste_mean"].values * 100, "s--", color=color,
                    linewidth=1.5, markersize=6, alpha=0.7, label=f"TD {dist_label} (×100)")
...
def gen_td_stats(td: pd.DataFrame, out_dir: Path) -> None:
    ...
    for ax, (metric, ylabel) in zip(axes, [
        ...
        for dist in sorted(td["dist"].unique()):
            ...
            ax.plot(sub["N"].values, sub[metric].values, "o-", color=DIST_COLORS.get(dist, "gray"),
                    linewidth=2, markersize=8, label=DIST_LABELS.get(dist, dist))
        ax.set_xlabel("Network size N", fontsize=10)
        ax.set_ylabel(ylabel, fontsize=10)
        ax.set_title(metric, fontsize=11)
        ax.legend(fontsize=9)
    ...
    for ax, metric in zip(axes2, ["waste_mean", "waste_std"], strict=True):
        for dist in sorted(td["dist"].unique()):
            ...
            ax.bar(np.arange(len(sub)) + offset, sub[metric].values, 0.35,
                   color=DIST_COLORS.get(dist, "gray"), alpha=0.85, label=DIST_LABELS.get(dist, dist))
    ...
    for ax, dist in zip(axes[0], dists, strict=True):
        ...
        bp = ax.boxplot(data, positions=range(len(data)), widths=0.6, patch_artist=True,
                        ...
                        flierprops=dict(marker=".", markersize=3, alpha=0.35,
                                        markerfacecolor=theme["muted"], markeredgecolor="none"))
...
def gen_npz_hist_kde(raw: dict[tuple, np.ndarray], theme: dict, out_dir: Path) -> None:
    ...
    for ax, dist in zip(axes[0], dists, strict=True):
        ...
        ax.hist(pooled, bins=60, density=True, color=DIST_COLORS.get(dist, "gray"), alpha=0.35,
                label="All sizes (pooled)")
...
def gen_npz_extended_stats(ext: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig.suptitle("Extended NPZ Statistics vs Network Size (lines: Rio Maior · diamonds: FFZ-350)",
                 fontsize=14, fontweight="bold")
...
def gen_dataset_interactive_html(npz: pd.DataFrame, td: pd.DataFrame, theme: dict, out_dir: Path) -> dict[str, Path]:
    ...
    for city in sub30["city_label"].unique():
        for dist in sub30["dist_label"].unique():
            ...
            fig.add_trace(go.Scatter(
                x=sub["mean_kg"], y=sub["std_kg"], mode="markers", name=f"{city} — {dist}",
                marker=dict(color=color, symbol=sym, size=(sub["N"] / 20 + 8).tolist(),
                            line=dict(width=1, color=theme["fg"]), opacity=0.85),
                text=hover, hovertemplate="%{text}<extra></extra>",
            ))
    fig.update_layout(
        title="NPZ Dataset Statistics — Mean vs Std Waste (30-day horizon)",
        xaxis_title="Mean waste (kg/bin/day)", yaxis_title="Std waste (kg/bin/day)",
        template=template, height=600, hovermode="closest", legend_title="City — Distribution",
    )
    ...
    fig2 = make_subplots(rows=1, cols=len(cities),
                         subplot_titles=[CITY_LABELS.get(c, c) for c in cities], shared_yaxes=True)
    ...
    for mi, metric in enumerate(metrics):
        ...
        for city in cities:
            ...
            for dist in sorted(sub30["dist"].unique()):
                ...
                if not len(sub):
                    continue
                hover = (city_l + "<br>N=" + sub["N"].astype(str) + "<br>Dist: " + dist_l
    ...
    fig3.update_layout(
        ...
        template=template, height=800, barmode="group", legend_title="City & Distribution",
    )
...
def build_npz_table(npz: pd.DataFrame, ext: pd.DataFrame, horizon: int = 30) -> str:
    ...
    rows = ["| City | N | Distribution | Mean kg | Median kg | Std kg | Max kg | IQR kg | Skewness |",
            "|------|---|-------------|---------|-----------|--------|--------|--------|---------|"]
...
def build_extended_table(ext: pd.DataFrame) -> str:
    ...
    rows = ["| City | N | Distribution | Median | Variance | Q1 | Q3 | IQR | Min | Fences (lo/hi) | Mode |",
            "|------|---|-------------|--------|----------|----|----|-----|-----|----------------|------|"]
