...
class VRPPContextEmbedder(ContextEmbedder):
    ...
    def init_node_embeddings(self, nodes: dict[str, Any], temporal_features: bool = True) -> torch.Tensor:
        ...
        # Standard: locs (N), depot (1). Combined: locs (N+1).
        # We assume they are customer-only if locs.size(1) matches waste.size(1)
        waste_check = nodes.get("waste")
        is_concatenated = False
        if waste_check is not None and locs.size(1) == waste_check.size(1) + 1:
