...
Shared encoder for the Multi-Decoder Attention Model with dynamic re-encoding
capability for mid-decode updates.
"""
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    This is used for the intermediate layers of the MDAM encoder.
    """
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            x: Input tensor of shape (batch_size, graph_size, embed_dim).
            mask: Optional attention mask.

        Returns:
            Output tensor of shape (batch_size, graph_size, embed_dim).
        """
...
class MDAMGraphAttentionEncoder(AutoregressiveEncoder):
    ...
    A shared encoder for all decoder paths that produces node embeddings.
    The last layer uses MDAM-specific MHA to return attention weights and
    value matrices for dynamic re-encoding during decoding.

    The `change()` method allows re-computing embeddings with masked attention,
    enabling the encoder to update embeddings as nodes are visited.
    ...
    def __init__(
        ...
        Args:
            ...
            node_dim: Input node feature dimension. If provided, a linear
                     projection is used to map to embed_dim.
            normalization: Type of normalization ('batch', 'layer', 'instance').
            feed_forward_hidden: Hidden dimension for feed-forward networks.
        """
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            x: Pre-computed input node embeddings. If None, uses td['observations'] or similar.
            mask: Optional attention mask (currently not supported).
            **kwargs: Additional arguments.

        Returns:
            Tuple of:
                - h: Final node embeddings (batch_size, graph_size, embed_dim)
                - graph_embed: Graph-level embedding (batch_size, embed_dim)
                - attn: Attention weights (num_heads, batch_size, graph_size, graph_size)
                - V: Value matrices (num_heads, batch_size, graph_size, embed_dim)
                - h_old: Pre-attention embeddings (batch_size, graph_size, embed_dim)
        """
        assert mask is None, "Mask not yet supported in MDAM encoder"

        # Use provided x or extract from td
        if x is None:
            # Placeholder for default feature extraction if x not provided
            # In MDAMPolicy, x is pre-computed via init_embed.
    ...
    def change(
        ...
        This method allows updating embeddings during decoding by masking
        out visited nodes in the attention computation.

        Args:
            attn: Attention weights (num_heads, batch_size, graph_size, graph_size).
            V: Value matrices (num_heads, batch_size, graph_size, embed_dim).
            h_old: Pre-attention embeddings (batch_size, graph_size, embed_dim).
            mask: Boolean mask indicating valid (unvisited) nodes
                  (batch_size, graph_size).

        Returns:
            Tuple of:
                - h: Updated node embeddings (batch_size, graph_size, embed_dim)
                - graph_embed: Updated graph embedding (batch_size, embed_dim)
        """
        num_heads, batch_size, graph_size, feat_size = V.size()

        # Apply mask to attention weights
        # mask: (batch_size, graph_size) -> (1, batch_size, 1, graph_size)
        ...
        # heads: (num_heads, batch_size, graph_size, feat_size)
        heads = torch.matmul(attn_masked, V)

        # Project and reshape
        # (batch_size, graph_size, num_heads, feat_size) -> (batch_size, graph_size, embed_dim)
        h_new = torch.mm(
