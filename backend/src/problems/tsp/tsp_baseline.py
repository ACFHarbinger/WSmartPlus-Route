...
def solve_all_nn(dataset_path, eval_batch_size=1024, no_cuda=False, dataset_n=None, progress_bar_mininterval=0.1):
    ...
    for batch in tqdm(dataloader, mininterval=progress_bar_mininterval):
        ...
        duration = time.time() - start
        results.extend([(cost.item(), np.trim_zeros(pi.cpu().numpy(), 'b'), duration) for cost, pi in zip(lengths, tours)])
...
if __name__ == "__main__":
    ...
    for dataset_path in opts.datasets:
        ...
        if method == "nn":
            ...
            results, parallelism = run_all_in_pool(run_func, target_dir, dataset, opts, use_multiprocessing=use_multiprocessing)
        else:
