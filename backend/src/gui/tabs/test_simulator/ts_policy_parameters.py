...
from backend.src.gui.app_definitions import DECODE_TYPES
...
class TestSimPolicyParamsTab(QWidget):
    ...
    def get_params(self):
        look_ahead_configs = []
        if self.lookahead_config_a.isChecked(): look_ahead_configs.append('a')
        if self.lookahead_config_b.isChecked(): look_ahead_configs.append('b')
        params = {
        ...
        if len(look_ahead_configs) > 0: params['lookahead_configs'] = " ".join(look_ahead_configs)
        if self.pregular_level_input.text().strip(): params["pregular_level"] = self.pregular_level_input.text().strip()
        if self.plastminute_cf_input.text().strip(): params["plastminute_cf"] = self.plastminute_cf_input.text().strip()
        if self.gurobi_param_input.text().strip(): params["gurobi_param"] = self.gurobi_param_input.text().strip()
        if self.hexaly_param_input.text().strip(): params["hexaly_param"] = self.hexaly_param_input.text().strip()
        return params
