...
from backend.src.utils.io_utils import compose_dirpath
...
def plot_linechart(output_dest, graph_log, plot_func, policies, x_label=None, y_label=None, title=None, fsave=True,
                   scale="linear", x_values=None, linestyles=None, markers=None, annotate=True, pareto_front=False):
            ...
            for lg in zip(*graph_log):
                for id, xy in enumerate(zip(list(zip(*lg))[0], list(zip(*lg))[5])):
                    ...
                    if id == graph_log.shape[0] - 1:
                        plt.scatter(xy[0], xy[1], s=200, marker='o', facecolors='none', 
    ...
    if pareto_front:
        ...
        for id_nbins, points in points_by_nbins.items():
            ...
            plt.plot(pareto_x, pareto_y, '--', color="black", linewidth=2, label=label, zorder=5)
