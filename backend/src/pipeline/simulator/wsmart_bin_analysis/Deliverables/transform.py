...
def fix_collections_sensor(container:Container, 
                           box_window:int, 
                           mv_thresh:int, 
                           min_days:int, 
                           dist_thresh:int,  
                           ...
                           var_thresh:int,
                           use:str,
                           spear_thresh:int=None) -> tuple[float,list[TAG],Container]:    
    """
    ...
    if tag != TAG.LOW_MEASURES:
        ...
        container.place_collections(dist_thresh=dist_thresh, spear_thresh=spear_thresh, c_trash=c_trash, max_fill=max_fill)
...
def filter_containers(containers_dict:dict) -> dict:
    ...
    """
    return dict(filter(lambda c: c[1].tag == TAG.OK or c[1].tag == None, containers_dict.items()))
...
def pre_process_container_metrics(containers_dict:dict, calc_spearman:bool = True):
    ...
    for _, container in containers_dict.items():
        ...
        if calc_spearman: container.calc_spearman()
...
def view_metrics(containers_dict:dict, box_window:int, mv_thresh:int, min_days:int, use:str):
    ...
    def make_histogram(data, bins, yaxis, tick_labels = None, histrange = None):
        ...
        if tick_labels == None:
            ...
            plt.bar(bin_centers, hist, width=(bin_edges[1] - bin_edges[0]), alpha=0.6, tick_label = tick_labels)
    ...
    make_histogram([tag.value for tag in tags], range(len(TAG)+1), 
                   "Percentage", tick_labels = [tag.name for tag in TAG],
