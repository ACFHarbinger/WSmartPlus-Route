...
def pre_process_data(
    ...
    """
    #Renaming
    rename_keys_fill = {date_header_fill: "Date", id_header_fill: "ID", fill_header_fill: "Fill"}
    ...
    info = df_collection.drop_duplicates(subset = 'ID'); info.drop(['Date'], axis=1)
    ...
    df_date_intersect = pd.DataFrame( {'f1': fill['Date'].first(),
...
def import_separate_file(src_fill: list[str], src_collect: list[str], sep_f:str = ',', sep_c:str = ',', path: str = "", print_firt_line = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def import_same_file(src_fill:str, collect_id_header:str, sep:str = ',', path:str = "", print_first_line = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
