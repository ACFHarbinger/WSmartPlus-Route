...
"""
THIS IS DEPRECATED PLEASE USE GRIDBASE FROM simulation.py FILE
"""


class OldGridBase():
    """
    Class that manages the bakground behind containers.
    ...
    def __init__(self, data_dir: str, area: str) -> None:
        ...
        self.__data:pd.DataFrame =  self.__data_preprocess(pd.read_csv(os.path.join(data_dir, "bins_waste", f"old_out_crude_rate[{self.src_area}].csv")))
        self.__info:pd.DataFrame = pd.read_csv(os.path.join(data_dir, "coordinates", f"old_out_info[{self.src_area}].csv"))
    ...
    def load_data(self, processed=True) -> tuple[pd.Timestamp]:
        if processed:
            ...
            data_file = pd.read_csv(os.path.join(self.data_dir, "bins_waste", f"old_out_crude_rate[{self.src_area}].csv"))
