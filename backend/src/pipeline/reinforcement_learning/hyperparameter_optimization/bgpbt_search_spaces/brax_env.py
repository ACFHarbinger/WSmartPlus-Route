...
from ..bgpbt_brax_train.default_configs import PPO_DEFAULT_CONFIGS
from .brax_trainer import get_brax_trainer


# Adapted from https://github.com/xingchenwan/bgpbt/blob/main/search_spaces/base.py
class SearchSpace:
    def __init__(self):
        self.config_space = None

    @abstractmethod
    def train_single(self, config: CS.Configuration, exp_idx: int, **kwargs):
        pass

    @abstractmethod
    def train_batch(self, configs: List[CS.Configuration], exp_idx_start: int, **kwargs):
        pass


# Adapted from https://github.com/xingchenwan/bgpbt/blob/main/search_spaces/brax_env.py
class Brax(SearchSpace):
        ...
        if do_nas:
            ...
            for net in nets:  # for sac, tune policy and action networks separately
                ...
                if choose_spectral_norm:
                    use_spectral_norm = CSHP.UniformIntegerHyperparameter(f'NAS_{net}_use_spectral_norm', 0, 1,
                                                                         default_value=1)
                    cs.add_hyperparameter(use_spectral_norm)
                if self.use_categorical:
                    num_hidden_layers = CSHP.CategoricalHyperparameter(f'NAS_{net}_num_layers', choices=list(
                        range(self.min_n_hidden_layers, self.max_n_hidden_layers + 1)))
                else:
                    num_hidden_layers = CSHP.UniformIntegerHyperparameter(f'NAS_{net}_num_layers', self.min_n_hidden_layers,
                                                                         self.max_n_hidden_layers, default_value=2)
                if self.smaller_network:
                    network_width = CSHP.CategoricalHyperparameter(f'NAS_{net}_log2_width', choices=[5, 6]) \
                        if self.use_categorical \
                        else CSHP.UniformIntegerHyperparameter(f'NAS_{net}_log2_width', 5, 6, default_value=5)
                else:
                    network_width = CSHP.CategoricalHyperparameter(f'NAS_{net}_log2_width', choices=[5, 6, 7, 8]) \
                        if self.use_categorical\
                        else CSHP.UniformIntegerHyperparameter(f'NAS_{net}_log2_width', 5, 8, default_value=5)
                cs.add_hyperparameter(num_hidden_layers)
                cs.add_hyperparameter(network_width)

        if do_hpo:
            if self.use_categorical:
                hyperparams = [CSHP.UniformFloatHyperparameter('log10_lr', -4, -3, default_value=np.log10(3e-4)),
                               CSHP.UniformFloatHyperparameter(
                    'discounting', 0.9, 0.9999, default_value=0.97, log=True),
                    CSHP.UniformFloatHyperparameter(
                    'log10_entropy_cost', -6, -1, default_value=-2),
                    CSHP.CategoricalHyperparameter('unroll_length', choices=list(
                        range(5, 16)), ),  # 5 - 15 inclusive
                    CSHP.CategoricalHyperparameter(
                ...
                hyperparams.append(CSHP.UniformFloatHyperparameter(
                ...
                hyperparams = [CSHP.UniformFloatHyperparameter('log10_lr', -4, -3, default_value=np.log10(3e-4)),
                    ...
                    CSHP.UniformFloatHyperparameter(
                    'log10_entropy_cost', -6, -1, default_value=-2),
                    CSHP.UniformIntegerHyperparameter(
                    ...
                    CSHP.UniformFloatHyperparameter(
                    'reward_scaling', 0.05, 20, default_value=10, log=True),
                    CSHP.UniformFloatHyperparameter('lambda_', 0.9, 1, default_value=0.95), ]
                hyperparams.append(CSHP.UniformFloatHyperparameter(
                    'ppo_epsilon', 0.1, 0.4, default_value=0.2))
    ...
    def _parse_config(self, config: CS.Configuration, return_str=True, return_dict=False, **kwargs):
        ...
        for k, v in config_dict.items():
            ...
            if 'log' in k:
                ...
                if type(self.config_space.get_hyperparameter(k)) in [CSHP.UniformIntegerHyperparameter,
                                                                     CSHP.NormalIntegerHyperparameter]:
    ...
    def train_batch(self, configs: List[CS.Configuration],
        ...
        for i_chunk in range(n_chunks):
            ...
            if len(current_chunk_strs) > 1:
                ...
                p = torch_mp.Pool(len(current_chunk_strs))
    ...
    def distill_batch(self, teacher_configs: List[CS.Configuration], student_configs: List[CS.Configuration],
        ...
        for i_chunk in range(n_chunks):
            ...
            p = torch_mp.Pool(len(current_chunk_teacher_dicts))
