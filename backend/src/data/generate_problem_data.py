...
from backend.src.utils.definitions import MAX_WASTE, MAX_LENGTHS, CAPACITIES
from backend.src.utils.functions import get_path_until_string
from backend.src.utils.data_utils import generate_waste_prize, load_focus_coords
from backend.src.pipeline.simulator.bins import Bins
from backend.src.pipeline.simulator.processor import process_coordinates
...
def generate_wcvrp_data(dataset_size, wcvrp_size, waste_type, 
    ...
    if focus_graph is not None:
        ...
        if distribution == 'emp':
            data_dir = get_path_until_string(focus_graph, 'wsr_simulator')
            bins = Bins(wcvrp_size, data_dir, sample_dist=distribution, area=area, indices=idx[0], grid=None)
        else:
            bins = None
    else:
        bins = None
